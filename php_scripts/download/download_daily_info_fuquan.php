...
function init_download($sina_url) {
...
function daily_download($sina_url) {
	echo date ( 'Y-m-d H:i:s', time () ) . "\n";
	echo "--------------------------START-----------------------------\n";
	...
	while ( $row = mysqli_fetch_array ( $query_code ) ) {
		...
		$jiduinfo = mb_convert_encoding ($jidu_content, "utf-8", "gb2312" );
			...
			if ($record_start) {
				...
				if ($line_number === 8) {
					...
					if ($df) {
						...
						// echo $trade_date . "\t" . $fqOpenPrice . "\t" . $fqClosePrice . "\t" . $fqHighestPrice . "\t" . $fqLowestPrice . "\t" . $trade_shares . "\t" . $trade_money . "\t" . $discount_factor . "\n";
						
						$openPrice = number_format(($fqOpenPrice/$discount_factor), 2, ".", "");
						$closePrice = number_format(($fqClosePrice/$discount_factor), 2, ".", "");
						$highestPrice = number_format(($fqHighestPrice/$discount_factor), 2, ".", "");
						$lowestPrice = number_format(($fqLowestPrice/$discount_factor), 2, ".", "");
						
						$insert_sql = "insert into stockdailyinfo_fq(stockcode, tradedate, openPrice, closePrice, highestPrice, lowestPrice, fqOpenPrice, fqClosePrice, fqHighestPrice, fqLowestPrice, tradeAmount, tradeMoney, discountFactor) " . "values('$code', $trade_date, $openPrice, $closePrice, $highestPrice, $lowestPrice, $fqOpenPrice, $fqClosePrice, $fqHighestPrice, $fqLowestPrice, $trade_shares, $trade_money, $discount_factor)";
...
//init_download($sina_url);
//get_years("<option value=\"2014\" >2014</option>");
//echo get_jidu(3);
daily_download($sina_url);
?>
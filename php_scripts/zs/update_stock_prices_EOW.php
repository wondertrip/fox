...
<?php
  ...
  for ($i = 0; $i < $arraylen; $i++){
    ...
    $prev_info1 = mysql_query("select * from stockdailyinfo_fq where stockcode = '$code1[1]' and tradedate >= $start_date order by tradedate asc limit 0,1");
    while ($prev_info1_row = mysql_fetch_array($prev_info1)) {
    	if ($prev_info1_row['openPrice'] != NULL) {
    		$new_start_price1 = number_format($prev_info1_row['openPrice'], 2);
    	}
    }
    echo $code1[1] . "\t start price:" . $new_start_price1."\n";
    ...
    $prev_info2 = mysql_query("select * from stockdailyinfo_fq where stockcode = '$code2[1]' and tradedate >= $start_date order by tradedate asc limit 0,1");
    while ($prev_info2_row = mysql_fetch_array($prev_info2)) {
    	if ($prev_info2_row['openPrice'] != NULL) {
    		$new_start_price2 = number_format($prev_info2_row['openPrice'], 2);
    	}
    }
    echo $code2[1] . "\t start price:" . $new_start_price2."\n";

...
<script type="text/javascript" class="init">
    $(document).ready(function() {
      $('#competition_tbl').dataTable( {
		...
		"sAjaxDataProp": "data",
		"sAjaxSource": "./php/zs_data.php?team=9",
		"bFilter": false,
		"bInfo": false,
		"bPaginate": false,
		"aoColumnDefs": [
	  ...
	  var request = $.ajax({
					url: "./php/zs_max_round.php?team=9",
					type: "GET",           
					dataType: "html"
				});
				...
				request.fail(function(jqXHR, textStatus) {
					$('#tbl_header').html("<h3>中暑山庄最强操盘手总表</h3>"); 
				});
	} );
    ...
    function showResult(tbl_name, optionValue, resetLable){
		...
		if (optionValue != 0){
			...
			url = url + "&with_round=true&team=8";
			...
			if (tbl_name == "history_of_num"){
			  ...
			  var request = $.ajax({
					url: "./php/get_round_date.php?team=8&round="+optionValue,
					type: "GET",           
					dataType: "html"
				});
				...
				request.fail(function(jqXHR, textStatus) {
					$('#history_by_num_header').html("<h3>我是操盘手短线选股游戏八强赛第" + optionValue + "周赛况表"); 
				});

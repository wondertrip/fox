...
<?php
  function isSuspended($prices){
    ...
    return ($prices[1] === "0.00" || $prices[1] === "0") && ($prices[3] === "0.00" || $prices[3] === "0") && ($prices[4] === "0.00" || $prices[4] === "0") && ($prices[5] === "0.00" || $prices[5] === "0");
  }
  echo date('Y-m-d H:i:s',time()) . "\n";
  echo "--------------------------START-----------------------------\n";
  $con = mysql_connect("localhost","root","Wind+3569");
  mysql_select_db("stock", $con);
  $max_round_result = mysql_query("select max(round) from trader_game");
  ...
  $all_stocks = mysql_query("select * from trader_game where round = $max_round");
  ...
  for ($i = 0; $i < $arraylen; $i++){
    ...
    if ($old_current_price_arr[0] != $prices1[3] && !isSuspended($prices1)) {
      $new_incr_rate1 = number_format(($prices1[3]-$start_price_arr[0])/$start_price_arr[0]*100, 2); 
    ...
    if ($old_highest_price_arr[0] < $prices1[4] && !isSuspended($prices1)){
      $new_highest_incr_rate1 = number_format(($prices1[4]-$start_price_arr[0])/$start_price_arr[0]*100, 2);
    ...
    $new_avg1 = number_format(($new_incr_rate1+$new_highest_incr_rate1)/2, 2);
    ...
    if ($old_current_price_arr[1] != $prices2[3] && !isSuspended($prices2)) {
      $new_incr_rate2 = number_format(($prices2[3]-$start_price_arr[1])/$start_price_arr[1]*100, 2);
    ...
    if ($old_highest_price_arr[1] < $prices2[4] && !isSuspended($prices2)){
      $new_highest_incr_rate2 = number_format(($prices2[4]-$start_price_arr[1])/$start_price_arr[1]*100, 2);
    ...
    $new_avg2 = number_format(($new_incr_rate2+$new_highest_incr_rate2)/2, 2);

    $new_score = number_format(($new_avg1+$new_avg2)/2, 2);
    ...
    $update_sql = "update trader_game set current_price=\"$new_current_price\", incr_rate=\"$new_incr_rate\", highest_price=\"$new_highest_price\", highest_incr_rate=\"$new_highest_incr_rate\", score=$new_score where round = $max_round and target_stock = \"$stock_codes[$i]\"";
    echo $update_sql . "\n";

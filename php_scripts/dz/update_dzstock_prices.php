...
<?php
  ...
  $all_stocks = mysql_query("select * from stock_dz_track");
  ...
  while($row = mysql_fetch_array($all_stocks)) {
	...
	$update_sql = "update stock_dz_track set current_price=$current_price, latest_incr_rate=$latest_incr_rate, incr_rate=$incr_rate where stockcode='$stockcode' and dz_date=$dz_date";
	echo $update_sql . "\n";

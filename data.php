...
define(LINK_TMPL, "<a href=\"./php/stock_info.php?code=sc1\">STOCK1</a><br><br><a href=\"./php/stock_info.php?code=sc2\">STOCK2</a>");
define(NAME_TMPL, "<a href=\"weibo_addr\">name</a>");
chdir(dirname(__FILE__));
require_once("./utils.php");

function formatStockLink($origi_str, $isLink = false) {
...
function getDataByRound($round, $con, $with_round){
  ...
  while($row = mysql_fetch_array($result)){
	if ($with_round == 'true') {
	  $record = array($row['round'], $row['num'], formatName($row['name'], $row['weibo']), $row['intro'], formatStockLink($row['target_stock'], true), formatStockLink($row['start_price']), formatStockLink($row['current_price']), formatStockLink($row['incr_rate']), formatStockLink($row['highest_price']), formatStockLink($row['highest_incr_rate']), $row['score'], $row['total_rounds'], $row['champion_num'], $row['total_score']);
	  array_push($rows, $record);
	} else {
	  $record = array($row['num'], formatName($row['name'], $row['weibo']), $row['intro'], formatStockLink($row['target_stock'], true), formatStockLink($row['start_price']), formatStockLink($row['current_price']), formatStockLink($row['incr_rate']), formatStockLink($row['highest_price']), formatStockLink($row['highest_incr_rate']), $row['score'], $row['total_rounds'], $row['champion_num'], $row['total_score']);
	  array_push($rows, $record);
	}
  }
